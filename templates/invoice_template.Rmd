---
params:
  invoice: "client/CloudArmy/invoice_20210212.yaml"
  client: "client/CloudArmy/cloud_army.yaml"
  provider: "provider/micaela.yaml"
mainfont: Open Sans
output: 
  pdf_document:
...
Balance Due:    \tabto{4cm} `r sprintf("$%.3f",balance_due)`\
...
t %>%
  ...
  kable(format="latex") %>%
  kable_styling(latex_options = "striped", stripe_index = c(1:nrow(t))) %>%
